...
public class TestCase {
	public static void main(String[] args) {
		...
		try {
			...
			freq = myObject.frequency();
			if(-1 != freq) {System.out.println("frequency() for hoge, should return -1, when taget is \"\". But it returns "+freq); c++; }

			myObject = new Frequencer();
			myObject.setSpace("hoge".getBytes());
			// myObject.setTarget("".getBytes());
			freq = myObject.frequency();
			if(-1 != freq) {System.out.println("frequency() for hoge, should return -1, when taget is (not set). But it returns "+freq); c++; }

			myObject = new Frequencer();
			myObject.setSpace("".getBytes());
			myObject.setTarget("hoge".getBytes());
			freq = myObject.frequency();
			if(0 != freq) {System.out.println("frequency() for \"\", should return -1, when taget is hoge. But it returns "+freq); c++; }

			myObject = new Frequencer();
			// myObject.setSpace("".getBytes());
			myObject.setTarget("hoge".getBytes());
			freq = myObject.frequency();
			if(0 != freq) {System.out.println("frequency() for (not set), should return -1, when taget is hoge. But it returns "+freq); c++; }

			myObject = new Frequencer();
			myObject.setSpace("AAAAAAA".getBytes());
			myObject.setTarget("AAA".getBytes());
			freq = myObject.frequency();
			if(5 != freq) {System.out.println("frequency() for AAAAAAA, should return 5, when taget is AAA. But it returns "+freq); c++; }
			...
			if(2 != freq) {System.out.println("frequency() for hogehogehugahuga, should return 2, when taget is hogehogehugahuga in (4, 8). But it returns "+freq); c++; }
