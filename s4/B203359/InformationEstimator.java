...
public class InformationEstimator implements InformationEstimatorInterface {
    ...
    public double estimation() {
        ...
        for (int i = 0; i < myTarget.length; i++) {
            iqMemo[i] = iq(myFrequencer.subByteFrequency(0, i+1));
            for (int j = 0; j < i; j++) {
                double tmpValue = iqMemo[j] + iq(myFrequencer.subByteFrequency(j+1, i+1));
                if (tmpValue < iqMemo[i]) iqMemo[i] = tmpValue;
            }
        }
        return iqMemo[myTarget.length-1];
    }
